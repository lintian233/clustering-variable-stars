...
def get_class_name(datasetpath):
    ...
    for class_name in class_dir:
        ...
        sub_class_name = [
            f
            for f in sub_class_name
            if os.path.isdir(os.path.join(datasetpath, class_name, f))
        ]
...
def generate_features(dataset_name_list, dataset_name):
    ...
    if threads < 4:
        print_red(
            "warning: the number of threads is less than 4, it will take a long time to generate features!"
        )
    ...
    while count + 4 < l:
        p1 = sp.Popen(
            f"python generate_features.py -c {dataset_name_list[count]} -d {dataset_name}",
            shell=True,
        )
        p2 = sp.Popen(
            f"python generate_features.py -c {dataset_name_list[count+1]} -d {dataset_name}",
            shell=True,
        )
        p3 = sp.Popen(
            f"python generate_features.py -c {dataset_name_list[count+2]} -d {dataset_name}",
            shell=True,
        )
        p4 = sp.Popen(
            f"python generate_features.py -c {dataset_name_list[count+3]} -d {dataset_name}",
            shell=True,
        )
        p1.wait()
        p2.wait()
...
def check_dir_exist():
    ...
    if dataset_name == "None":
        print_red(
            "error: please specify the dataset that you want to process!",
            file=sys.stderr,
        )
        exit(errno.ENOENT)
...
def check_feature_exist_and_generate(dataset_name):
    ...
    if if_exist_feature:
        ...
        print_red(
            "Warning: it will take a long time to regenerate features!(about 20 hours or more)"
        )
