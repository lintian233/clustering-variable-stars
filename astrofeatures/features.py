...
class OlgeFeatures:
    ...
    def _get_indexof_log_span(self, data, bin):
        ...
        value_arr = np.array(
            [(10 ** (log_start + i * log_step)) for i in range(1, bin + 1)]
        )
        index_arr = []
    ...
    def _generate_features_90(self):
        ...
        for i in range(len(bins_idx) - 1):
            current_peak = np.int_(
                peaks[(peaks > bins_idx[i]) & (peaks < bins_idx[i + 1])]
            )
...
class AstroDataFeatures:
    ...
    def INIT(self):
        ...
        percent_of_meadian_range_1_over_10 = self._percent_of_meadian_range_1_over_10(
            self.mag
        )
        ...
        ampij, phij, harmonic_mag, fitting_mag = self._amp_ph_of_harmonic_fit_lc(
            self.time, self.mag
        )
        self.log10_harmonic_amplitude_ratio = self._log10_harmonic_amplitude_ratio(
            ampij
        )
        self.log10_fit_residual_raw_iqr_lc = self._log10_fit_residual_raw_iqr_lc(
            self.mag, fitting_mag
        )
        self.abbe_of_harmonic_fit_lc = self._abbe_of_residuals_har_fit(
            self.mag, harmonic_mag, fitting_mag
        )
        ...
        log10_fit_residual_raw_iqr_lc = self._log10_fit_residual_raw_iqr_lc(
            self.mag, fitting_mag
        )
        abbe_of_harmonic_fit_lc = self._abbe_of_residuals_har_fit(
            self.mag, harmonic_mag, fitting_mag
        )
    ...
    def _percent_of_meadian_range_1_over_10(self, mag):
        ...
        mag_median_range_count = np.sum(
            (mag_sort > mag_median - mag_range_tenth) & (mag_sort < mag_median_range)
        )
    ...
    def __excess_abbe(self, x, y, window_size):
        ...
        for i in range(first, end):
            indexes = np.where(
                (x > x[i] - 0.5 * window_size) & (x < x[i] + 0.5 * window_size)
            )
            if len(indexes[0]) < 5:
                continue
    ...
    def __whiten(self, x, y):
        ...
        whitening_matrix = np.dot(
            np.dot(eigenvectors, np.diag(1.0 / np.sqrt(eigenvalues))), eigenvectors.T
        )
    ...
    def _amp_ph_of_harmonic_fit_lc(self, time, mag):
        ...
        def harmonic_func(x, a1, a2, a3, a4, b1, b2, b3, b4, b0):
            ...
            y = np.zeros(len(x))
            for i in range(4):
                y += a[i] * np.sin(2 * np.pi * best_f * (i + 1) * x) + b[i] * np.cos(
                    2 * np.pi * best_f * (i + 1) * x
                )
            y += b0
            return y
        ...
        def best_func(x, *params):
            def _harmonic_func(x, a1, a2, a3, a4, b1, b2, b3, b4, best_f, b0):
                ...
                y = np.zeros(len(x))
                for i in range(4):
                    y += a[i] * np.sin(2 * np.pi * best_f * (i + 1) * x) + b[
                        i
                    ] * np.cos(2 * np.pi * best_f * (i + 1) * x)
                y += b0
                return y
        ...
        new_popt, new_pcov = curve_fit(
            best_func, time, mag, maxfev=1000000, p0=init_params
        )
    ...
    def _log10_fit_residual_raw_iqr_lc(self, mag, fitting_mag):
        ...
        residuals_iqr = np.percentile(sorted_residuals, 75) - np.percentile(
            sorted_residuals, 25
        )
